...
export default class ArrivalsData {
    ...
    public async getArrivalsData( status_header_data: StatusHeaderDataType, set_arrivals_callback: Dispatch<SetStateAction<ArrivalsDataType>>, update_arrmaxy_callback: (retArrMaxY: number) => void ) {
        ...
        const get_arrivals_url = "http://localhost:8080/arrivals/" + start_date + "/" + end_date + "/" + status_header_data.door_to_provider + "/" + status_header_data.group_name + "/" + status_header_data.facility_name + "/" + status_header_data.department_name;
        
        const res = await fetch(get_arrivals_url);
